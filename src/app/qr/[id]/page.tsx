  ...
  if (loading) {
    return (
      <main className="min-h-screen flex items-center justify-center" style={{ background: "#EEF1EE" }}>
        <p style={{ color: "#6B7570" }}>Memuat data peralatan…</p>
      </main>
    );
  }
  ...
  if (!equipment) {
    return (
      <main className="min-h-screen flex items-center justify-center p-4" style={{ background: "#EEF1EE" }}>
        <div className="bg-white rounded-xl border p-8 text-center max-w-md" style={{ borderColor: "#D7DCD7" }}>
          <p className="text-4xl mb-2">❓</p>
          <h1 className="font-semibold text-lg mb-1">Peralatan tidak ditemukan</h1>
          <p className="text-sm mb-4" style={{ color: "#6B7570" }}>
            QR code ini tidak valid atau peralatan sudah dihapus.
          </p>
          <button onClick={() => router.push("/login")} className="text-sm font-medium px-4 py-2 rounded-md text-white" style={{ background: "#2F5D62" }}>
            ← Ke dashboard
          </button>
        </div>
      </main>
        ...
        <div className="p-6">
          ...
          <div className="space-y-2 mb-6">
            <div className="flex justify-between py-2 border-b" style={{ borderColor: "#EEF1EE" }}>
              <span className="text-xs font-medium uppercase" style={{ color: "#8A9590" }}>Tipe</span>
              <span className="text-sm">{equipment.type}</span>
            </div>
            <div className="flex justify-between py-2 border-b" style={{ borderColor: "#EEF1EE" }}>
              <span className="text-xs font-medium uppercase" style={{ color: "#8A9590" }}>Lokasi</span>
              <span className="text-sm">{equipment.location || "-"}</span>
            </div>
            <div className="flex justify-between py-2 border-b" style={{ borderColor: "#EEF1EE" }}>
              <span className="text-xs font-medium uppercase" style={{ color: "#8A9590" }}>Interval</span>
              <span className="text-sm">{equipment.service_interval_days} hari</span>
            </div>
            <div className="flex justify-between py-2">
              <span className="text-xs font-medium uppercase" style={{ color: "#8A9590" }}>Service terakhir</span>
              <span className="text-sm">{equipment.last_service_date || "Belum pernah"}</span>
            </div>
          </div>

          <button
            onClick={handleService}
            className="w-full text-white font-medium py-3 px-4 rounded-md transition hover:opacity-90"
          ...
          <button
            ...
            className="w-full text-sm font-medium py-2 mt-2 rounded-md hover:opacity-75"
            style={{ color: "#2F5D62" }}
          >
